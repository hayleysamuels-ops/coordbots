...
function createSchedulingStore({ databaseUrl, dir, pool: suppliedPool }) {
  if (databaseUrl || suppliedPool) {
    ...
    const ready = pool.query("CREATE TABLE IF NOT EXISTS scheduling_proposals (" +
      "id TEXT PRIMARY KEY, revision INTEGER NOT NULL, state TEXT NOT NULL, data JSONB NOT NULL)").then(() =>
      pool.query("CREATE UNIQUE INDEX IF NOT EXISTS scheduling_one_active_candidate ON scheduling_proposals " +
        "((data->'plan'->>'candidateId')) WHERE state IN ('draft','approved','running','needs_review')")).then(() =>
      pool.query("CREATE UNIQUE INDEX IF NOT EXISTS scheduling_one_execution ON scheduling_proposals ((1)) WHERE state='running' OR (state='needs_review' AND data->>'issue'='booking_uncertain')"));
    ready.catch(() => {}); // Requests still await ready and surface database failure.
    return {
      async list() { await ready; return (await pool.query("SELECT data FROM scheduling_proposals ORDER BY id")).rows.map((r) => r.data); },
      async get(id) { await ready; return (await pool.query("SELECT data FROM scheduling_proposals WHERE id=$1", [id])).rows[0]?.data || null; },
      async insert(proposal) {
        await ready;
        return (await pool.query("INSERT INTO scheduling_proposals(id,revision,state,data) VALUES($1,$2,$3,$4) ON CONFLICT DO NOTHING",
      ...
      async replace(id, revision, next) {
        ...
        try {
          return (await pool.query("UPDATE scheduling_proposals SET revision=$3,state=$4,data=$5 WHERE id=$1 AND revision=$2",
  ...
  return {
    ...
    async insert(proposal) {
      if (rows[proposal.id] || Object.values(rows).some(row =>
        ["draft", "approved", "running", "needs_review"].includes(row.state) &&
