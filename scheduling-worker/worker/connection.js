...
function createConnection({ chromium, vault, chromiumSandbox = true, now = () => Date.now() }) {
  ...
  return {
    ...
    async finish(owner,id) {
      ...
      // This exact account/organization label was observed in Poetic's UI. It
      // establishes session identity only, not readiness to book.
      if(await page.getByRole("button",{name:"Tess Wicks Poetic",exact:true}).count()!==1) fail(409,"Open Poetic in Tess Wicks's Ashby account before saving");
      vault.save(await live.context.storageState({indexedDB:true}));
    ...
    async status() { return {sessionSaved:!!vault.load(),bookingEnabled:false,signInOpen:!!lease && lease.expires>now()}; },
    close,
  };
}
