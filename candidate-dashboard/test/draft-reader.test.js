...
function setup(){
 ...
 const page={goto:async target=>{visits.push(target);url=options.redirect?'https://app.ashbyhq.com/login':target;},url:()=>url, getByRole:(role,query)=>({waitFor:async()=>{},first:()=>({waitFor:async()=>{}}),isChecked:async()=>true,click:async()=>{url='https://app.ashbyhq.com/schedules/drafts/'+input.draftId;},all:async()=>[{getAttribute:async()=>options.wrongCandidate?'/other':'/candidates/'+input.candidateId+'/applications/'+input.applicationId}]}),getByPlaceholder:()=>({waitFor:async()=>{}}),getByText:()=>({waitFor:async()=>{}}),locator:()=>({last:()=>({waitFor:async()=>{}}),innerText:async()=>url.endsWith('calendar-invites')?'Candidate Invite\nVirtual welcome\nInterviewer Invite\nMary only':(options.loading?'PREVIEW\nLoading template builder':'PREVIEW\nFrom Anna\nHi TEST')})};
...
test('calendar inspection explicitly navigates from Communications to Schedule',async()=>{const s=setup();const r=await createDraftReader(s.deps).inspect({...input,interviewer:{name:'Mary',email:'mary@example.com'}},'calendar');assert.equal(s.visits[0],'https://app.ashbyhq.com/schedules/drafts/'+input.draftId+'/communication/calendar-invites');assert.equal(r.availabilityVerified,false);assert.equal(r.observedBusy.length,1);assert.equal(s.closed(),1);});
