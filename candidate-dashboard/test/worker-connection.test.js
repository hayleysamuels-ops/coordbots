...
const {signed,verifier}=require("../src/scheduling/worker-auth");
...
test("sign-in lease belongs to one coordinator and saving requires configured identity",async t=>{
 const f=fake(),c=createConnection({...f,clientId:"test",expectedIdentity:"Test Coordinator Test Client"});t.after(()=>c.close());const session=await c.start("owner@carrara.is");
...
test("expired login cannot be used and navigation refuses unknown destinations",async t=>{
 const f=fake();let clock=100;const c=createConnection({...f,clientId:"test",expectedIdentity:"Test Coordinator Test Client",now:()=>clock});t.after(()=>c.close());const session=await c.start("owner@carrara.is");
...
test("browser sandbox is the default and alternative isolation must be server-configured",async t=>{
 const strict=fake(),normal=createConnection({...strict,clientId:"test",expectedIdentity:"Test Coordinator Test Client"});t.after(()=>normal.close());await normal.start("owner@carrara.is");assert.equal(strict.launchOptions.chromiumSandbox,true);
 const alternative=fake(),explicit=createConnection({...alternative,clientId:"test",expectedIdentity:"Test Coordinator Test Client",chromiumSandbox:false});t.after(()=>explicit.close());await explicit.start("owner@carrara.is");assert.equal(alternative.launchOptions.chromiumSandbox,false);
});
...
test("session status is client-bound and does not claim current verification",async t=>{
 const f=fake(),c=createConnection({...f,clientId:"client-a",expectedIdentity:"Coordinator Client A"});t.after(()=>c.close());
 const lease=await c.start("owner");await c.finish("owner",lease.id);
 assert.equal(f.saved.clientId,"client-a");assert.equal((await c.status()).sessionSaved,true);
 assert.equal((await c.status()).sessionVerified,false);assert.equal((await c.status()).bookingEnabled,false);
 const other=createConnection({...f,clientId:"client-b",expectedIdentity:"Coordinator Client B"});
 assert.equal((await other.status()).sessionSaved,false);
});
test("unconfigured worker refuses startup",()=>{
 assert.throws(()=>createConnection(fake()),/required/);
});

test("only deployment configuration selects isolation; booking runtime is absent",()=>{
